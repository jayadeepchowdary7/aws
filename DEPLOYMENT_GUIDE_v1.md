...
Happy deploying! 🚀
## Helm Usage with Argo CD

Helm is a package manager for Kubernetes that allows you to define, install, and upgrade applications using Helm charts.

In the context of Argo CD, you can use Helm to manage your application deployments. Argo CD can sync Helm charts directly from a Git repository.

### Using Helm with Argo CD

1. **Package your application as a Helm chart**:
   - Create a `Chart.yaml` file with metadata.
   - Define your templates in the `templates/` directory.
   - Optionally, create a `values.yaml` for default configuration.

2. **Store the Helm chart in your Git repository**:
   - You can store the chart in a directory like `helm/` or `charts/` in your repo.
   - Ensure that the chart is versioned (you can use the chart's version or the app's version).

3. **Configure Argo CD to sync the Helm chart**:
   - When creating the Argo CD application, specify the path to the chart directory.
   - Argo CD will use `helm template` to render the manifest and apply it to the cluster.

4. **Updating the image tag**:
   - If you are using a Helm chart, you can set the image tag in the `values.yaml` or pass it as a parameter.
   - In your Jenkins pipeline, after pushing the image, you can update the `values.yaml` file with the new image tag and commit the change.

   Example of updating the image tag in values.yaml (if using Helm):

   ```bash
   # In Jenkins pipeline, after pushing the image
   sed -i \"s|tag: .*|tag: ${IMAGE_TAG}|g\" helm/spring-boot-app/values.yaml
   git add helm/spring-boot-app/values.yaml
   git commit -m \"chore: update image tag to ${IMAGE_TAG} [skip ci]\"
   git push origin HEAD:${GIT_BRANCH:-main}
   ```

### Verifying Helm Releases with Argo CD

- In the Argo CD UI, you can see the Helm release status.
- You can also use the `argocd` CLI to get the application details and see the synced resources.

### Benefits

- **Templating**: Helm allows you to parameterize your manifests (e.g., image tag, replica count).
- **Rollbacks**: Helm keeps a history of releases, making rollbacks easy.
- **Dependency Management**: Helm charts can depend on other charts.

For more information, see the [Helm documentation](https://helm.sh/docs/).
## GitOps with Argo CD
